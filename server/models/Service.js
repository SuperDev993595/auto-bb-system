...
const workOrderSchema = new mongoose.Schema(
  {
    ...
    vehicle: {
      _id: {
        type: mongoose.Schema.Types.ObjectId,
        ref: "Vehicle",
        required: false, // Optional since we can have embedded vehicle data
      },
      make: {
        type: String,
        required: true,
...
serviceCatalogSchema.virtual("totalPrice").get(function () {
  ...
  const partsCost = this.parts
    ? this.parts.reduce((sum, part) => sum + part.totalPrice, 0)
    : 0;
...
workOrderSchema.methods.calculateTotals = function () {
  this.totalLaborHours = this.services
    ? this.services.reduce((sum, service) => sum + service.laborHours, 0)
    : 0;
  this.totalLaborCost = this.services
    ? this.services.reduce(
        (sum, service) => sum + service.laborHours * service.laborRate,
        0
      )
    : 0;
  this.totalPartsCost = this.services
    ? this.services.reduce(
        (sum, service) =>
          sum +
          (service.parts
            ? service.parts.reduce(
                (partSum, part) => partSum + part.totalPrice,
                0
              )
            : 0),
        0
...
serviceSchema.virtual("summary").get(function () {
  return `${
    this.serviceType
  } - ${this.date.toLocaleDateString()} - $${this.total.toFixed(2)}`;
});
