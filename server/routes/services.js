...
const workOrderSchema = Joi.object({
  ...
  vehicle: Joi.object({
    _id: Joi.string().optional(), // Optional vehicle ID for referencing separate vehicle document
...
const workOrderUpdateSchema = Joi.object({
  ...
  estimatedStartDate: Joi.alternatives()
    .try(Joi.date(), Joi.string().allow("", null))
    .optional(),
  estimatedCompletionDate: Joi.alternatives()
    .try(Joi.date(), Joi.string().allow("", null))
    .optional(),
  actualStartDate: Joi.alternatives()
    .try(Joi.date(), Joi.string().allow("", null))
    .optional(),
  actualCompletionDate: Joi.alternatives()
    .try(Joi.date(), Joi.string().allow("", null))
    .optional(),
  notes: Joi.string().allow("", null).optional(),
  customerNotes: Joi.string().allow("", null).optional(),
...
const basicServiceSchema = Joi.object({
  ...
  status: Joi.string()
    .valid("completed", "in-progress", "scheduled", "cancelled")
    .default("completed"),
...
router.get("/catalog/public", async (req, res) => {
  try {
    ...
    const services = await ServiceCatalog.find(query).sort(sort).exec();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await workOrderService.createFromAppointment(
        appointmentId,
        req.user.id
      );
...
router.put("/workorders/:id", requireAnyAdmin, async (req, res) => {
  try {
    ...
    await workOrder.populate(
      "vehicle",
      "make model year vin licensePlate mileage color"
    );
    if (workOrder.technician) {
      await workOrder.populate("technician", "name email");
    }
...
// @access  Private
router.post(
  "/workorders/:id/assign-technician",
  requireAnyAdmin,
  async (req, res) => {
    try {
...
router.get("/catalog/:id", requireAnyAdmin, async (req, res) => {
  try {
    const service = await ServiceCatalog.findById(req.params.id).populate(
      "createdBy",
      "name email"
    );
...
router.get("/catalog/stats/overview", requireAnyAdmin, async (req, res) => {
  try {
    ...
    const activeServices = await ServiceCatalog.countDocuments({
      isActive: true,
    });
    const inactiveServices = await ServiceCatalog.countDocuments({
      isActive: false,
    });
...
router.get("/workorders/stats/overview", requireAnyAdmin, async (req, res) => {
  try {
    ...
    const pendingCount = await WorkOrder.countDocuments({
      ...query,
      status: "pending",
    });
    ...
    const cancelledCount = await WorkOrder.countDocuments({
      ...query,
      status: "cancelled",
    });
    const onHoldCount = await WorkOrder.countDocuments({
      ...query,
      status: "on_hold",
    });
...
router.get("/technicians/stats/overview", requireAnyAdmin, async (req, res) => {
  try {
    ...
    const inactiveTechnicians = await Technician.countDocuments({
      isActive: false,
    });
...
router.get("/:id", requireAnyAdmin, async (req, res) => {
  try {
    ...
    let service = await ServiceCatalog.findById(req.params.id).populate(
      "createdBy",
      "name email"
    );
...
router.post("/:id/pricing", requireAnyAdmin, async (req, res) => {
  try {
    ...
    if (
      laborRate !== undefined &&
      (typeof laborRate !== "number" || laborRate < 0)
    ) {
      return res.status(400).json({
        success: false,
...
router.put("/workorders/:id/start", requireAnyAdmin, async (req, res) => {
  try {
    ...
    const result = await workOrderService.startWork(
      req.params.id,
      technicianId
    );
...
router.put("/workorders/:id/progress", requireAnyAdmin, async (req, res) => {
  try {
    ...
    const result = await workOrderService.updateProgress(
      req.params.id,
      progress,
      notes
    );
...
router.put("/workorders/:id/check-parts", requireAnyAdmin, async (req, res) => {
  try {
    ...
    const partsAvailability = await workOrderService.checkPartsAvailability(
      allParts
    );
