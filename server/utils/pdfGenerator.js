...
class PDFGenerator {
  ...
  async generateInvoicePDF(invoice) {
    ...
    this.doc.setFontSize(14);
    this.doc.setFont("helvetica", "bold");
    this.doc.text(`INVOICE #${invoice.invoiceNumber || "N/A"}`, 120, yPosition);
    yPosition += 8;
    ...
    const issueDate = invoice.issueDate || invoice.date || new Date();
    const dueDate = invoice.dueDate || new Date();
    const status = invoice.status || "pending";

    this.doc.text(
      `Date: ${moment(issueDate).format("MM/DD/YYYY")}`,
      120,
      yPosition
    );
    yPosition += 5;
    ...
    yPosition += 5;
    this.doc.text(`Status: ${status.toUpperCase()}`, 120, yPosition);
    yPosition += 15;
    ...
    if (invoice.customerId && typeof invoice.customerId === "object") {
      this.doc.text(invoice.customerId.name || "N/A", 20, yPosition);
      yPosition += 5;

      if (invoice.customerId.address) {
        // Handle address as object or string
        let addressText = "";
        if (typeof invoice.customerId.address === "object") {
      ...
      // Fallback if customerId is not populated
      this.doc.text("Customer information not available", 20, yPosition);
      yPosition += 15;
    }

    // Vehicle information
    if (invoice.vehicleId && typeof invoice.vehicleId === "object") {
      this.doc.setFontSize(12);
      this.doc.setFont("helvetica", "bold");
      ...
      const year = invoice.vehicleId.year || "N/A";
      const make = invoice.vehicleId.make || "N/A";
      const model = invoice.vehicleId.model || "N/A";
      this.doc.text(`${year} ${make} ${model}`, 20, yPosition);
      yPosition += 5;
      ...
      if (invoice.vehicleId.licensePlate) {
        this.doc.text(
          `License: ${invoice.vehicleId.licensePlate}`,
          20,
          yPosition
        );
        yPosition += 5;
    ...
    if (invoice.items && invoice.items.length > 0) {
      invoice.items.forEach((item) => {
        ...
        this.doc.text(description, 20, yPosition);
        this.doc.text(quantity.toString(), 100, yPosition);
        this.doc.text(`$${unitPrice.toFixed(2)}`, 130, yPosition);
      ...
      this.doc.text(`$${total.toFixed(2)}`, 130, yPosition);
      this.doc.text(`$${total.toFixed(2)}`, 160, yPosition);
      yPosition += 6;
    }
    ...
    if (paidAmount > 0) {
      this.doc.setFontSize(10);
      this.doc.setFont("helvetica", "normal");
      this.doc.text(`Amount Paid: $${paidAmount.toFixed(2)}`, 20, yPosition);
      yPosition += 5;
      this.doc.text(
        `Balance Due: $${(total - paidAmount).toFixed(2)}`,
        20,
        yPosition
      );
      yPosition += 10;
  ...
  async generateDailyActivityReport(userId, date = new Date()) {
    ...
    taskTypes.forEach((type) => {
      ...
      const completedTypeTasks = typeTasks.filter(
    ...
    Object.entries(typeStats).forEach(([type, stats]) => {
      ...
      this.doc.text(
        `${typeName}: ${stats.completed}/${stats.total} (${stats.percentage}%)`,
        20,
        yPosition
      );
      yPosition += 5;
    ...
    if (tasks.length > 0) {
      ...
      tasks.forEach((task, index) => {
        ...
        yPosition += 4;
        this.doc.text(
          `   Type: ${task.type} | Status: ${task.status} | Due: ${moment(
            task.dueDate
          ).format("MM/DD/YYYY")}`,
          20,
          yPosition
        ...
        if (task.customer) {
          this.doc.text(
            `   Customer: ${task.customer.businessName}`,
            20,
            yPosition
          );
          yPosition += 4;
  ...
  async generateCustomerReport(customerId) {
    ...
    yPosition += 5;
    this.doc.text(
      `Contact Person: ${customer.contactPerson.name}`,
      20,
      yPosition
    );
    yPosition += 5;
    ...
    yPosition += 5;
    this.doc.text(
      `Email: ${customer.contactPerson.email || "N/A"}`,
      20,
      yPosition
    );
    yPosition += 5;
    ...
    if (customer.communicationLogs.length > 0) {
      ...
      customer.communicationLogs.slice(0, 10).forEach((log, index) => {
        if (yPosition > 250) {
          this.doc.addPage();
          this.addHeader(
            `Customer Report - ${customer.businessName} (Continued)`
          );
          yPosition = 60;
        }

        this.doc.text(
          `${index + 1}. ${moment(log.createdAt).format(
            "MM/DD/YYYY HH:mm"
  ...
  async generateWorkCompletionSummary(customerId, date = new Date()) {
    ...
    if (completedTasks.length === 0) {
      ...
      completedTasks.forEach((task, index) => {
        if (yPosition > 250) {
          this.doc.addPage();
          this.addHeader(
            `Work Completion Summary - ${customer.businessName} (Continued)`
          );
          yPosition = 60;
        }
    ...
    this.doc.text(
      "This report was automatically generated by the Auto Repair CRM system.",
      20,
      yPosition
    );
    yPosition += 5;
    this.doc.text(
      "For questions or concerns, please contact your assigned representative.",
      20,
      yPosition
    );
  ...
  async generateSuperAdminDailyReport(date = new Date()) {
    ...
    this.doc.setFont("helvetica", "bold");
    this.doc.text(
      `Daily Report - ${targetDate.format("MMMM Do YYYY")}`,
      20,
      yPosition
    );
    yPosition += 15;
    ...
    for (const admin of subAdmins) {
      const adminTasks = allTasks.filter(
        (task) => task.assignedTo._id.toString() === admin._id.toString()
      );
      ...
      this.doc.setFont("helvetica", "normal");
      this.doc.text(
        `   Tasks: ${adminCompletedTasks.length}/${adminTasks.length} (${
          adminTasks.length > 0
            ? Math.round((adminCompletedTasks.length / adminTasks.length) * 100)
            : 0
        }%)`,
        20,
        yPosition
